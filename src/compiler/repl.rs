        ...
        if result.is_ok() {
            ...
            for (name, definition) in functions {
                ...
                if result.is_err() {
                    ...
                    self.type_info.function_types.insert(name, return_type);
                }
            }
        } else {
        ...
        match expression {
            ...
            Expression::FunctionCall(function_call) => match function_call.name.as_str() {
                ...
                _ => {
                    ...
                    let definition = self.program.functions.get(&function_call.name).unwrap();
                                ...
                                match arg_type {
                                    Type::SimpleType(simple_type) => {
                                        ...
                                        if !self.program.types.contains_key(simple_type) {
                                            return Err(format!(
                                                "Type '{simple_type}' is not defined"
                                            ));
                                        }
                                        let definition =
                                            self.program.types.get(simple_type).unwrap();
