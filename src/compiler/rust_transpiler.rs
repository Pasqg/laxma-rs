...
fn compile_function(
    ...
    let return_type = match return_type.unwrap() {
        Type::FunctionType(_, return_type) => return_type,
        _ => return Err(format!("Expected FunctionType but got '{:?}'", return_type.unwrap())),
    };
